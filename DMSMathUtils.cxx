...
float DMS_MathUtils::GaussianKernel(float x, float xi, float bandwidth) {
  float diff = x - xi;/*{{{*/
  return exp( -0.5 * ( diff * diff ) / ( bandwidth * bandwidth ));
}/*}}}*/

float DMS_MathUtils::GaussianKernelRegression(const std::vector<float>& x_data, const std::vector<float>& y_data, float x, float bandwidth) {
...
TH1D* DMS_MathUtils::PerformKDE(TH1D* hist, int nSmoothBins, float bandwidth) {
...
bool DMS_MathUtils::GetPulseRange(float threshold, float* start, float* end, TH1D* hist, float searchStart, float searchEnd)
{/*{{{*/
  bool inPulse = false;
  bool found = false;
  float x, y, y_next;
...
std::pair<float, float> DMS_MathUtils::GetMeanAndVariance(TH1D* hist, int startBin, int endBin)
{// {{{
  float sum = 0.0f;
...
float DMS_MathUtils::GetPedestal(TH1D* hist, float start_ns, float end_ns)
{// {{{
...
float DMS_MathUtils::GetCherenkovPedestal(TH1D* hist)
{// {{{
  int endbin = 30;
  float average = 0;
  float stddev = 0;
...
TH1D* DMS_MathUtils::GetDerivative(TH1D* hist)
{// {{{
...
int DMS_MathUtils::GetNumberOfZeroCrossing(TH1D* hist)
{
  return 0;
}
